 ...
 * MC_DEV_SERVICE picks the compose service (default "minecraft"; "minecraft-lab" for the second server).
...
export async function rcon(command) {
    const { stdout } = await run(
        'docker',
        ['compose', '-f', 'docker-compose.dev.yml', 'exec', '-T', process.env.MC_DEV_SERVICE ?? 'minecraft', 'rcon-cli', command],
